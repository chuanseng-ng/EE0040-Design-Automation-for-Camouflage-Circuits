...
class Connector :
    ...
    def set (self, value) :
        ...
        if self.monitor : print('Connector: %s-%s set to %d') %(self.owner.name, self.name, self.value)
...
#Comment out plotting function as it is not needed in current scope
	...
	# 			if self.pclk:
	# 				plt.subplot(plts+1,1,i)
	# 			else:
	# 				plt.subplot(plts,1,i)
	# 			num = range(len(p))
	# 			plt.step(num,p)
	...
	# 				k=0
	# 				for j in p:
	# 					plt.annotate("%d" %(j), xy=(k-0.5,j))
	# 					k=k+1

	# 			plt.set_title("%s" %(self.pnames[i-1]))
	# 			plt.ylim(-0.5,1.5)
	# 			plt.xlim(-1, len(p))
	# 			i=i+1
	...
	# 			plt.step(num,self.clk)

	# 			if self.pannotate:
	# 				kk=0
	# 				for jj in self.clk:
	# 					plt.annotate("%d" %(jj), xy=(kk-0.5,jj))
	# 					kk=kk+1
	# 			plt.set_title("clock")
	# 			plt.ylim(-0.5,1.5)
	# 			plt.xlim(-1,len(self.clk))
